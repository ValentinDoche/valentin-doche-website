<?php
function admin_post_modify($post){ ?>
    <body>
    <?=admin_sidebar("post-modify")?>
    <?=admin_header("Post Update ID : {$post['id']}")?>
    <main>
        <div class="container">
            <form>
                <div class="row">
                    <div class="input-field col s12">
                        <input id="post_title" type="text" class="validate" required value="<?=$post['title']?>">
                        <label for="post_title">Post Title</label>
                    </div>
                </div>
                <div class="row">
                    <div class="input-field col s12">
                        <input id="post_short_description" type="text" class="validate" required value="<?=$post['short_description']?>">
                        <label for="post_short_description">Post Short Description</label>
                    </div>
                </div>
                <div class="row">
                    <div class="input-field col s12">
                        <input id="post_meta_description" type="text" class="validate" required value="<?=$post['meta_description']?>">
                        <label for="post_meta_description">Post Meta Description</label>
                    </div>
                </div>
                <div class="row">
                    <div class="input-field col s12 m6">
                        <input id="post_slug" type="text" class="validate" required value="<?=$post['slug']?>">
                        <label for="post_slug">Post Slug</label>
                    </div>
                    <div class="input-field col s12 m6">
                        <input id="post_image" type="text" class="validate" required value="<?=$post['image']?>">
                ...
                <div class="row">
                    <div class="input-field col s12 m6">
                        <label>
                            <input type="checkbox" id="post_publish" <?=($post['publish'] == 1 ? 'checked' : 'unchecked')?> </>
                            <span>Publish</span>
                        </label>
                    </div>
                    <div class="input-field col s12 m6">
    ...
    <script>
                ...
                $.post(
                    ...
                    function (data) {
                        if (data === 'success'){
                            ...
                            M.toast({html: "Error cant update post", classes: 'red darken-1 rounded'})
                        }
                    }
                )
            })
        ...
        function setContent(){
            $.post(
                ...
                function (data) {
                    if (data !== 'error'){
                        tinyMCE.get('post_content').setContent(data)
                    }else {
